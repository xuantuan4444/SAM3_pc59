...
================================
Standalone script version of val_train_unetaspp_pc59_llm.ipynb.
...
its own bare name), while the trained SAM3+UNet+ASPP model (no DINOv2) hybrid-refines only the
N target classes it was trained on (read dynamically from the checkpoint, NOT hardcoded here).
At every pixel where UNet+ASPP predicts something other than "other", the result overwrites the
SAM3 baseline; everywhere else, the SAM3 baseline is kept unchanged.

PC59 uses a 0-58/255-void label convention (no valid "background" class in the label space --
see sam3_baseline_pc59_llm.py for why): sam3_pred defaults to 255 (no confident detection), NOT
0, since 0 is a real class ("aeroplane").
...
Expected project layout (all paths resolved from this script's own location):
    ...
    |-- val_train_unetaspp_pc59_llm.py          <- this file
    |-- data_pc59/
    |   |-- JPEGImages/
    ...
    |-- weight_sam3/sam3.pt
    `-- weights_unetaspp_pc59_llm_v1/unetaspp_pc59_llm_v1_best.pth   <- output of train_unetaspp_pc59_llm.py
...
Usage:
    ...
    python val_train_unetaspp_pc59_llm.py
...
UNET_CKPT_PATH = PROJECT_ROOT / "weights_unetaspp_pc59_llm_v1" / "unetaspp_pc59_llm_v1_best.pth"  # TODO: adjust -- output of train_unetaspp_pc59_llm.py
...
if __name__ == "__main__":
    try:
        ...
        print("\n[FATAL] val_train_unetaspp_pc59_llm.py failed:", file=sys.stderr)
        traceback.print_exc()
        sys.exit(1)
