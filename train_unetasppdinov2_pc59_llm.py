...
==================================
Standalone script version of train_unetasppdinov2_pc59_llm.ipynb.

Trains the SAM3+UNet+ASPP+DINOv2 refinement network on PASCAL-Context 59, LLM prompt-arm coarse
masks. Adds a frozen DINOv2 ViT-S/14 feature branch (cached to disk once, since each image is
visited across many training epochs) on top of train_unetaspp_pc59_llm.py's architecture. Logic
is unchanged from the notebook -- only I/O has been adapted for a plain VM with no Jupyter
(plt.show() -> saved PNG, no notebook magics). Module-level globals mirror the notebook's own
cell-to-cell state exactly, same convention as train_unetaspp_pc59_llm.py.

--------------------------------------------------------------------------------------------
Expected project layout (all paths resolved from this script's own location):

    pc59_ablation/                              <- PROJECT_ROOT
    |-- train_unetasppdinov2_pc59_llm.py        <- this file
    |-- data_pc59/
    |   |-- JPEGImages/
    |   |-- SegmentationClassContext/
    |   `-- adjust_prompt_pc59.json             <- single source of truth for TARGET_CLASSES
    `-- coarse_cache_pc59/                      <- output of get_coarse_pc59_llm.py (has split.json)
...
Output (written under PROJECT_ROOT/weights_aspp_pc59_llm_v1/):
    unet_aspp_pc59_llm_v1_best.pth
    unet_aspp_pc59_llm_v1_last.pth
    unet_aspp_pc59_llm_v1_history.json
    training_curves.png

Also writes a DINOv2 feature cache under PROJECT_ROOT/dinov2_cache_pc59/ (one .npz per image,
reused across epochs and across reruns).
...
Usage:
    ...
    python train_unetasppdinov2_pc59_llm.py
...
COARSE_CACHE_DIR = PROJECT_ROOT / "coarse_cache_pc59"  # TODO: adjust -- output of get_coarse_pc59_llm.py
SPLIT_JSON = COARSE_CACHE_DIR / "split.json"
DINOV2_CACHE_DIR = PROJECT_ROOT / "dinov2_cache_pc59"

WEIGHTS_DIR = PROJECT_ROOT / "weights_aspp_pc59_llm_v1"
BEST_CKPT = WEIGHTS_DIR / "unet_aspp_pc59_llm_v1_best.pth"
LAST_CKPT = WEIGHTS_DIR / "unet_aspp_pc59_llm_v1_last.pth"
HISTORY_JSON = WEIGHTS_DIR / "unet_aspp_pc59_llm_v1_history.json"
...
def train_model(model, train_loader, val_loader, optimizer, scheduler, scaler, autocast_ctx):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        ckpt = dict(
            ...
            arch_version="multi_class_softmax_pc59_llm_v1_dinov2",
        )
...
if __name__ == "__main__":
    try:
        ...
        print("\n[FATAL] train_unetasppdinov2_pc59_llm.py failed:", file=sys.stderr)
        traceback.print_exc()
        sys.exit(1)
