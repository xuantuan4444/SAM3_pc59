...
==================================
Standalone script version of val_train_unetaspp_pc59_nollm.ipynb.

"Hybrid" evaluation: SAM3 runs the full 59-class evaluation using each class's bare name
(no-LLM arm -- CLASS_PROMPTS_OVERRIDE is empty), while the trained SAM3+UNet+ASPP model
(no DINOv2, no-LLM coarse masks) hybrid-refines only the N target classes it was trained on
(read dynamically from the checkpoint, NOT hardcoded here). At every pixel where UNet+ASPP
predicts something other than "other", the result overwrites the SAM3 baseline; everywhere
else, the SAM3 baseline is kept unchanged.
...
Expected project layout (all paths resolved from this script's own location):
    ...
    |-- val_train_unetaspp_pc59_nollm.py        <- this file
    |-- data_pc59/
    |   |-- JPEGImages/
    |   |-- SegmentationClassContext/
    |   `-- pascal_context_val.txt
    |-- weight_sam3/sam3.pt
    `-- weights_unetaspp_pc59_nollm_v1/unetaspp_pc59_nollm_v1_best.pth   <- output of train_unetaspp_pc59_nollm.py
...
Usage:
    ...
    python val_train_unetaspp_pc59_nollm.py
...
UNET_CKPT_PATH = PROJECT_ROOT / "weights_unetaspp_pc59_nollm_v1" / "unetaspp_pc59_nollm_v1_best.pth"  # TODO: adjust -- output of train_unetaspp_pc59_nollm.py
...
def load_prompts_and_val_ids():
    ...
    the N ablation target classes the UNet+ASPP was trained to refine."""
...
def load_unet_checkpoint():
    ...
    print(f"Loaded UNet+ASPP checkpoint: {UNET_CKPT_PATH}")
    ...
    print(f"  IN_CHANNELS_BASE   : {IN_CHANNELS_BASE}  (no DINOv2 at this ablation level)")
...
def predict_hybrid_semantic_map(image_path):
    ...
    UNet+ASPP branch (no DINOv2): only active on the N classes in TARGET_CLASSES. The result
...
if __name__ == "__main__":
    try:
        ...
        print("\n[FATAL] val_train_unetaspp_pc59_nollm.py failed:", file=sys.stderr)
        traceback.print_exc()
        sys.exit(1)
