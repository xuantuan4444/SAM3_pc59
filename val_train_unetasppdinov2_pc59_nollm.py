...
=========================================
Standalone script version of val_train_unetasppdinov2_pc59_nollm.ipynb.

"Hybrid" evaluation: SAM3 runs the full 59-class evaluation using each class's bare name
(no-LLM arm -- CLASS_PROMPTS_OVERRIDE is empty), while the trained SAM3+UNet+ASPP+DINOv2 model
(no-LLM coarse masks) hybrid-refines only the N target classes it was trained on (read
dynamically from the checkpoint, NOT hardcoded here, DINOv2 config included). At every pixel
where the refine network predicts something other than "other", the result overwrites the SAM3
baseline; everywhere else, the SAM3 baseline is kept unchanged.

Unlike train_unetasppdinov2_pc59_nollm.py, DINOv2 features are computed ON-THE-FLY per image
(no cache) -- val images are each visited once, so caching would add complexity for no benefit.
...
Expected project layout (all paths resolved from this script's own location):
    ...
    |-- val_train_unetasppdinov2_pc59_nollm.py  <- this file
    |-- data_pc59/
    |   |-- JPEGImages/
    |   |-- SegmentationClassContext/
    |   `-- pascal_context_val.txt
    |-- weight_sam3/sam3.pt
    `-- weights_aspp_pc59_nollm_v1/unet_aspp_pc59_nollm_v1_best.pth   <- output of train_unetasppdinov2_pc59_nollm.py
...
Usage:
    ...
    python val_train_unetasppdinov2_pc59_nollm.py
...
UNET_CKPT_PATH = PROJECT_ROOT / "weights_aspp_pc59_nollm_v1" / "unet_aspp_pc59_nollm_v1_best.pth"  # TODO: adjust -- output of train_unetasppdinov2_pc59_nollm.py
...
def load_prompts_and_val_ids():
    """No-LLM arm: every one of the 59 PC59 classes queries SAM3 with its own bare name --
    CLASS_PROMPTS_OVERRIDE stays empty, so CLASS_PROMPTS_OVERRIDE.get(cname, [cname]) in
    predict_hybrid_semantic_map() always falls through to [cname] for every class, including
    the N ablation target classes the UNet+ASPP+DINOv2 was trained to refine."""
    global CLASS_PROMPTS_OVERRIDE, val_ids

    CLASS_PROMPTS_OVERRIDE = {}
    print("No-LLM arm: CLASS_PROMPTS_OVERRIDE is empty -- every class prompts SAM3 with its bare name.")
...
if __name__ == "__main__":
    try:
        ...
        print("\n[FATAL] val_train_unetasppdinov2_pc59_nollm.py failed:", file=sys.stderr)
        traceback.print_exc()
        sys.exit(1)
