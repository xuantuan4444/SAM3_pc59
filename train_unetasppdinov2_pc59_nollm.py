...
=====================================
Standalone script version of train_unetasppdinov2_pc59_nollm.ipynb.

Trains the SAM3+UNet+ASPP+DINOv2 refinement network on PASCAL-Context 59, no-LLM (bare
class-name prompt) coarse masks. Adds a frozen DINOv2 ViT-S/14 feature branch (cached to disk
once) on top of train_unetaspp_pc59_nollm.py's architecture.
...
source of truth and can never drift out of sync. The only real difference from the LLM-arm
script is COARSE_CACHE_DIR below, which points at coarse masks built from bare class-name SAM3
queries instead of the ensemble.

--------------------------------------------------------------------------------------------
Expected project layout (all paths resolved from this script's own location):

    pc59_ablation/                              <- PROJECT_ROOT
    |-- train_unetasppdinov2_pc59_nollm.py      <- this file
    |-- data_pc59/
    |   |-- JPEGImages/
...
from a local repo/checkpoint if torch.hub.load fails (see load_dinov2_backbone() below for the
fallback search paths -- edit them to match wherever you placed a local copy).

Output (written under PROJECT_ROOT/weights_aspp_pc59_nollm_v1/):
    unet_aspp_pc59_nollm_v1_best.pth
    unet_aspp_pc59_nollm_v1_last.pth
    unet_aspp_pc59_nollm_v1_history.json
    training_curves.png

Also writes a DINOv2 feature cache under PROJECT_ROOT/dinov2_cache_pc59_nollm/ (one .npz per
image, reused across epochs and across reruns).
...
Usage:
    ...
    python train_unetasppdinov2_pc59_nollm.py
...
DINOV2_CACHE_DIR = PROJECT_ROOT / "dinov2_cache_pc59_nollm"

WEIGHTS_DIR = PROJECT_ROOT / "weights_aspp_pc59_nollm_v1"
BEST_CKPT = WEIGHTS_DIR / "unet_aspp_pc59_nollm_v1_best.pth"
LAST_CKPT = WEIGHTS_DIR / "unet_aspp_pc59_nollm_v1_last.pth"
HISTORY_JSON = WEIGHTS_DIR / "unet_aspp_pc59_nollm_v1_history.json"
...
def load_target_config():
    """No-LLM arm: TARGET_CLASSES still sourced from adjust_prompt_pc59.json's keys (SAME file
    as the LLM-arm script) -- this script never reads the prompt-ensemble VALUES (only
    get_coarse_pc59_{llm,nollm}.py do), only the class-name keys. TVERSKY_PARAMS /
    ...
    global NUM_TARGETS, NUM_OUT_CLASSES, IN_CHANNELS_BASE, IN_CHANNELS_TOTAL, TVERSKY_PARAMS, BOUNDARY_LAMBDA
    ...
    IN_CHANNELS_TOTAL = IN_CHANNELS_BASE + DINOV2_COMPRESS
    ...
    print(f"DINOv2        : {DINOV2_MODEL_NAME}, embed_dim={DINOV2_EMBED_DIM}, input={DINOV2_INPUT_SIZE}x{DINOV2_INPUT_SIZE}, grid={DINOV2_GRID_SIZE}x{DINOV2_GRID_SIZE}")
...
def build_dinov2_cache(internal_train_ids, internal_val_ids):
    """Each image -> forward DINOv2 frozen -> save features [384, 37, 37] at
    dinov2_cache_pc59/{image_id}.npz (key 'features'). Skip if file already exists with proper
    shape. Filters internal_train_ids/internal_val_ids down to only images with a valid cache
    entry, exactly like the notebook."""
    DINOV2_CACHE_DIR.mkdir(parents=True, exist_ok=True)
    all_dino_targets = internal_train_ids + internal_val_ids
    ...
    try:
        ...
        if repo_path is None:
            raise RuntimeError(
                ...
                "Enable internet access, or clone https://github.com/facebookresearch/dinov2 "
                "into one of the paths above."
            )
    ...
    for img_id in tqdm(all_dino_targets, desc="Build DINOv2 cache"):
        out_path = DINOV2_CACHE_DIR / f"{img_id}.npz"
        if out_path.exists():
            try:
        ...
        try:
            image_pil = Image.open(PC59_IMAGES_ROOT / f"{img_id}.jpg")
            feat = dinov2_forward(image_pil)
            np.savez_compressed(out_path, features=feat)
            n_new += 1
        except Exception as e:
            dino_errors.append((img_id, str(e)))

    print(f"\nDINOv2 cache: {n_cached} already existed, {n_new} new, {len(dino_errors)} errors")
    if dino_errors:
        for e in dino_errors[:5]:
            print(f"  {e}")
    ...
    def _dino_ok(img_id):
        p = DINOV2_CACHE_DIR / f"{img_id}.npz"
        if not p.exists():
            return False
        try:
            with np.load(p) as d:
    ...
    internal_train_ids = [i for i in internal_train_ids if _dino_ok(i)]
    internal_val_ids = [i for i in internal_val_ids if _dino_ok(i)]
    print(f"Final internal-train: {len(internal_train_ids)}   internal-val: {len(internal_val_ids)}")
...
class SAM3PC59Dataset(Dataset):
    ...
    Returns:
      ...
      dino_feat    : [384, 37, 37] -- DINOv2 feature of the ORIGINAL image (before aug, except HFlip).
    ...
    def _load_dinov2(self, img_id):
        with np.load(self.dinov2_dir / f"{img_id}.npz") as d:
    ...
    def __getitem__(self, idx):
        ...
        dino_feat = self._load_dinov2(img_id)                                 # (384, 37, 37) f32
        ...
        # Augment: RGB + masks (label + valid + coarses). HFlip is done manually to stay in sync
        # with dino_feat.
...
def build_datasets(internal_train_ids, internal_val_ids):
    train_ds = SAM3PC59Dataset(internal_train_ids, COARSE_CACHE_DIR, DINOV2_CACHE_DIR, aug=build_augment_no_hflip())
    val_ds = SAM3PC59Dataset(internal_val_ids, COARSE_CACHE_DIR, DINOV2_CACHE_DIR, aug=None)
...
class ResNet34UNetASPPPC59(nn.Module):
    ...
    DINOv2 branch (learned): 1x1 conv 384 -> 32 -> upsample bilinear -> [B, 32, H, W]
    ...
    def forward(self, input_base, dino_feat):
        """
        input_base: [B, 3+NUM_TARGETS, H, W]  (H=W=512)
        ...
        x = torch.cat([input_base, dino_u], dim=1)  # [B, total_in, H, W]
...
def train_model(model, train_loader, val_loader, optimizer, scheduler, scaler, autocast_ctx):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        ckpt = dict(
            ...
            arch_version="multi_class_softmax_pc59_nollm_v1_dinov2",
        )
...
def plot_curves(history, best_epoch, best_miou, out_path):
    ...
    plt.suptitle(f"UNet+ASPP+DINOv2 PC59 -- Best Epoch {best_epoch} | Internal-val mIoU {best_miou:.4f}", fontsize=14)
    plt.tight_layout()
...
def main():
    ...
    internal_train_ids, internal_val_ids = build_dinov2_cache(internal_train_ids, internal_val_ids)
...
if __name__ == "__main__":
    try:
        ...
        print("\n[FATAL] train_unetasppdinov2_pc59_nollm.py failed:", file=sys.stderr)
        traceback.print_exc()
        sys.exit(1)
