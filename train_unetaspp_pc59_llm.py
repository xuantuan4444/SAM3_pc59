...
============================
Standalone script version of train_unetaspp_pc59_llm.ipynb.

Trains the SAM3+UNet+ASPP refinement network (no DINOv2) on PASCAL-Context 59, LLM prompt-arm
coarse masks. Logic is unchanged from the notebook -- only I/O has been adapted for a plain VM
with no Jupyter (plt.show() -> saved PNG, no notebook magics). Module-level globals mirror the
notebook's own cell-to-cell state exactly (TARGET_CLASSES, CE_WEIGHTS, etc. are set once early
in main() and read by the Dataset/loss/model code that follows) -- this favors a low-risk,
literal translation of the verified notebook logic over a more heavily parameterized rewrite.

--------------------------------------------------------------------------------------------
Expected project layout (all paths resolved from this script's own location):

    pc59_ablation/                              <- PROJECT_ROOT
    |-- train_unetaspp_pc59_llm.py              <- this file
    |-- data_pc59/
    |   |-- JPEGImages/
...
Output (written under PROJECT_ROOT/weights_unetaspp_pc59_llm_v1/):
    unetaspp_pc59_llm_v1_best.pth
    unetaspp_pc59_llm_v1_last.pth
    unetaspp_pc59_llm_v1_history.json
    training_curves.png
...
Usage:
    ...
    python train_unetaspp_pc59_llm.py
...
WEIGHTS_DIR = PROJECT_ROOT / "weights_unetaspp_pc59_llm_v1"
BEST_CKPT = WEIGHTS_DIR / "unetaspp_pc59_llm_v1_best.pth"
LAST_CKPT = WEIGHTS_DIR / "unetaspp_pc59_llm_v1_last.pth"
HISTORY_JSON = WEIGHTS_DIR / "unetaspp_pc59_llm_v1_history.json"
...
def train_model(model, train_loader, val_loader, optimizer, scheduler, scaler, autocast_ctx):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        ckpt = dict(
            ...
            arch_version="multi_class_softmax_pc59_unetaspp_llm_v1",
        )
...
if __name__ == "__main__":
    try:
        ...
        print("\n[FATAL] train_unetaspp_pc59_llm.py failed:", file=sys.stderr)
        traceback.print_exc()
        sys.exit(1)
