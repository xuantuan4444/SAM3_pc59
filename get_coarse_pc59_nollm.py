...
=========================
Standalone script version of get_coarse_pc59_nollm.ipynb.

Precomputes SAM3 coarse binary masks (no-LLM, plain class-name prompts) for the PASCAL-Context
59 ablation target classes, over the official PC59 train split (4998 images, listed in
pascal_context_train.txt), then packages the resulting cache directory into a .zip.

Unlike VOC/Cityscapes' no-LLM scripts, which hardcode a literal target-class list, PC59's target
classes are not yet decided -- TARGET_CLASSES here is STILL sourced from adjust_prompt_pc59.json's
keys (the SAME file the LLM-arm script reads), so the two arms can never drift out of sync. This
script simply discards the prompt-ensemble VALUES and always queries SAM3 with the bare class
name (class_prompts = [[c] for c in target_classes]).
...
from the current working directory or any environment variable -- run it from anywhere):
    ...
    |-- get_coarse_pc59_nollm.py                <- this file
    |-- data_pc59/
    ...
    |   `-- adjust_prompt_pc59.json               <- TODO: create once target classes are chosen (same file the LLM-arm script uses)
    `-- weight_sam3/
...
Usage (from any directory):
    ...
    python /path/to/pc59_ablation/get_coarse_pc59_nollm.py [--rebuild-cache]
...
COARSE_CACHE_DIR = PROJECT_ROOT / "coarse_cache_pc59_nollm"
COARSE_CACHE_ZIP = PROJECT_ROOT / "coarse_cache_pc59_nollm.zip"
...
def load_prompt_ensemble(adjust_prompt_path: Path):
    """No-LLM arm: TARGET_CLASSES still sourced from adjust_prompt_pc59.json's keys (SAME file as
    the LLM arm) -- discards the prompt-ensemble VALUES entirely, always querying SAM3 with the
    bare class name."""
    ...
    class_prompts = [[c] for c in target_classes]  # no-LLM: bare name only
...
def build_coarse_cache(image_ids, target_classes, class_prompts, coarse_cache_dir: Path,
    ...
    errors = []
    try:
        for step, img_id in enumerate(tqdm(missing, desc="SAM3 coarse cache (PC59, no-LLM)"), start=1):
...
def audit_and_rebuild_cache(all_ids, target_classes, class_prompts, coarse_cache_dir: Path,
    ...
    for img_id in tqdm(all_ids, desc="Audit coarse_cache_pc59_nollm"):
...
def parse_args():
    p = argparse.ArgumentParser(description="Precompute SAM3 coarse masks for PC59 (no-LLM arm).")
...
def main():
    ...
    visualize_sample(internal_train_ids, target_classes, COARSE_CACHE_DIR,
                      COARSE_CACHE_DIR.parent / "sample_coarse_masks_pc59_nollm.png")
...
if __name__ == "__main__":
    try:
        ...
        print("\n[FATAL] get_coarse_pc59_nollm.py failed:", file=sys.stderr)
        traceback.print_exc()
        sys.exit(1)
