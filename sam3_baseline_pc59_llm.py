...
==========================
Standalone script version of sam3_baseline_pc59_llm.ipynb.

Evaluates SAM3 on the PASCAL-Context 59-class val split (5105 images), using the LLM-generated
prompt ensemble (adjust_prompt_pc59.json) for whichever target classes have an entry -- every
other class falls back to its own bare name, identical to sam3_baseline_pc59_nollm.py. Logic is
otherwise unchanged from sam3_baseline_pc59_nollm.py (same GT-convention diagnostic, same
metrics); only I/O has been adapted for a plain VM with no Jupyter (plt.show()/display() ->
saved PNG/CSV files, paths -> PROJECT_ROOT-relative).

--------------------------------------------------------------------------------------------
Expected project layout (all paths resolved from this script's own location):

    pc59_ablation/
    |-- sam3_baseline_pc59_llm.py         <- this file
    |-- data_pc59/
    ...
    |   `-- adjust_prompt_pc59.json        <- TODO: does not exist yet, see note below
    `-- weight_sam3/sam3.pt

adjust_prompt_pc59.json does not exist yet (target classes not chosen until
sam3_baseline_pc59_nollm.py's per-class IoU ranking is available) -- this script still runs fine
without it: falls back to an empty CLASS_PROMPTS_OVERRIDE (i.e. identical behavior to the no-LLM
baseline) until you generate it. Re-run this script once the file exists to get the real
LLM-arm baseline numbers.
...
Usage:
    ...
    python sam3_baseline_pc59_llm.py
...
# LLM prompt ensemble, loaded from adjust_prompt_pc59.json (generated by
# generate_adjust_prompt.py from contexts_class.json via Gemini, once target classes are
# chosen). Any PC59 class with no entry falls back to its own name as the single prompt.
...
DEVICE = None
processor = None
CLASS_PROMPTS_OVERRIDE = None  # populated in load_classes_and_split()
...
def predict_semantic_map_native(image_path, class_names):
    """
    Use Sam3Processor to predict a semantic label map, LLM prompt-ensemble version.

    For each class, query every prompt in its ensemble (CLASS_PROMPTS_OVERRIDE, falls back to
    [class_name] when the class has no LLM-generated entry). Every returned mask, regardless of
    which prompt or class produced it, competes in the SAME score-based conflict resolution as
    the no-LLM baseline.
...
def load_classes_and_split():
    global PC59_CLASSES, NUM_CLASSES, INDEX_TO_CLASS, CLASS_PROMPTS_OVERRIDE
    ...
    if ADJUST_PROMPT_PATH.exists():
        ...
        print(f"[warn] {ADJUST_PROMPT_PATH} not found -- every class falls back to its own name "
              "as the single prompt (identical to the no-LLM baseline until you create this file).")
        adjust_prompt = {}
    ...
    print(f"Classes without an LLM ensemble (fallback to own name): "
          f"{[c for c in PC59_CLASSES if c not in CLASS_PROMPTS_OVERRIDE]}")
...
if __name__ == "__main__":
    try:
        ...
        print("\n[FATAL] sam3_baseline_pc59_llm.py failed:", file=sys.stderr)
        traceback.print_exc()
        sys.exit(1)